#!/usr/bin/env bash
# vcan_usb functional test.
#
# Builds + reloads the driver, auto-detects every SocketCAN interface bound to
# vcan_usb (no fixed channel count — works for 1/2/3/4+ independent channels),
...
#
# The current vcan hardware is 2-channel, so this runs a 2x2 bus matrix when both
# ports are wired together.
#
#   sudo ./test_vcan_usb.sh
#   sudo env NFRAMES=100 ./test_vcan_usb.sh
#   sudo env BUILD=0 ./test_vcan_usb.sh       # skip rebuild/reload
#
# This script lives under kernel/tests/, separate from the DKMS-packaged
# driver source under kernel/vcan_usb/; it cd's into the driver directory to
# build/insmod there.
set -uo pipefail
cd "$(dirname "$0")/../../vcan_usb"
...
count_frames(){ # $1=receiver $2=id $3=classic|fd $4=brs
  awk -v r="$1" -v id="$2" -v kind="$3" -v brs="$4" '
    ...
    kind == "fd" && index($3, id "##") == 1 {
      # Match BRS bit only.  New kernels also set CANFD_FDF (bit 2), so
      # candump normally prints ##4/##5 instead of the submitted ##0/##1.
...
# unique payload of $1 bytes: even bytes = sender index, odd bytes = sequence
build_data(){
...
phase(){ # $1=classic|fd $2=brs(0|1) $3=label
  ...
  # Both tested ports are the physical ends of this two-channel bus.
  for i in "${!IFACES[@]}"; do
...
if [[ "$BUILD" == 1 ]]; then
  ...
  # insmod does not resolve module dependencies for an out-of-tree module.
...
detect_ifaces
if (( ${#IFACES[@]} == 0 )); then
  echo "no $DRIVER interfaces found. Is the device plugged in and the module loaded?"
  exit 1
fi
for i in "${!IFACES[@]}"; do
